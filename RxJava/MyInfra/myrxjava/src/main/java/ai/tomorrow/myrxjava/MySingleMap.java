...
public final class MySingleMap<T, R> extends MySingle<R> {
  ...
  public MySingleMap(
      MySingleSource<? extends T> source, MyFunction<? super T, ? extends R> mapper) {
    this.source = source;
    this.mapper = mapper;
  }
  ...
  static final class MyMapSingleObserver<T, R> implements MySingleObserver<T> {
    ...
    public MyMapSingleObserver(
        MySingleObserver<? super R> observer, MyFunction<? super T, ? extends R> mapper) {
